...
class Lynda(ProgressBar):
                    ...
                    for stream in lecture_streams:
                        ...
                        if content_length != 0:
                            ...
                            sys.stdout.write('\t- ' + fg + sd + "{:<23} {:<8}{}{}{}{}\n".format(
                                str(stream), str(stream.dimention[1]) + 'p', sz, in_MB, fy, sb))
    ...
    def download_assets(self, assets='', filepath=''):
        if assets:
            ...
            sys.stdout.write(fc + sd + "[" + fm + sb + "*" + fc +
                             sd + "] : " + fg + sd + "Downloading (%s)\n" % (title))
            try:
            ...
            if msg == 'already downloaded':
                sys.stdout.write(fc + sd + "[" + fm + sb + "*" + fc + sd + "] : " + fg +
                                 sd + "Asset : '%s' " % (title) + fy + sb + "(already downloaded).\n")
            elif msg == 'download':
                sys.stdout.write(
    ...
    def download_subtitles(self, subtitle='', filepath=''):
        if subtitle:
            ...
            filename = "%s\\%s" % (
                filepath, subtitle.filename) if os.name == 'nt' else "%s/%s" % (filepath, subtitle.filename)
            try:
        ...
        if lecture_best:
            ...
            if msg == 'already downloaded':
                sys.stdout.write(fc + sd + "[" + fm + sb + "*" + fc + sd + "] : " + fg + sd +
                                 "Lecture : '%s' " % (lecture_title) + fy + sb + "(already downloaded).\n")
            elif msg == 'download':
                sys.stdout.write(fc + sd + "[" + fm + sb + "+" + fc + sd +
                                 "] : " + fg + sd + "Downloaded  (%s)\n" % (lecture_title))
            else:
                sys.stdout.write(fc + sd + "[" + fm + sb + "*" + fc + sd + "] : " + fg + sd +
                                 "Lecture : '%s' " % (lecture_title) + fc + sb + "(download skipped).\n")
                sys.stdout.write(
                    fc + sd + "[" + fr + sb + "-" + fc + sd + "] : " + fr + sd + "{}\n".format(msg))
    ...
    def download_lectures_only(self, lecture_best='', lecture_title='', inner_index='', lectures_count='', filepath=''):
        if lecture_best:
            self.download_lectures(lecture_best=lecture_best, lecture_title=lecture_title,
                                   inner_index=inner_index, lectures_count=lectures_count, filepath=filepath)
...
def main():
    ...
    parser.add_argument(
        'course', help="Lynda course or file containing list of courses.", type=str)
        ...
        for course in courses:
            ...
            if not options.cookies:
                if not options.username and not options.password:
                    username = fc + sd + \
                        "[" + fm + sb + "*" + fc + sd + "] : " + \
                        fg + sd + "Username : " + fg + sb
                    password = fc + sd + \
                        "[" + fm + sb + "*" + fc + sd + "] : " + \
                        fg + sd + "Password : " + fc + sb
                    email = getpass.getuser(prompt=username)
                    passwd = getpass.getpass(prompt=password)
                    if email and passwd:
                        lynda = Lynda(url=course, username=email,
                                      password=passwd, organization=options.org)
                    else:
                        sys.stdout.write(
                    ...
                    lynda = Lynda(url=course, username=options.username,
                                  password=options.password, organization=options.org)
                    if options.info:
                        lynda.course_list_down()
    ...
    if not os.path.isfile(options.course):
        ...
        if not options.cookies:
            if not options.username and not options.password:
                ...
                passwd = getpass.getpass(prompt=password)
                if email and passwd:
                    lynda = Lynda(url=options.course, username=email,
                                  password=passwd, organization=options.org)
                else:
                    sys.stdout.write(
                        '\n' + fc + sd + "[" + fr + sb + "-" + fc + sd + "] : " + fr + sb + "Username and password is required.\n")
